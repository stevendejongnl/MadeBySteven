...
class FetchGitLabContributions:
    """Use case: Fetch GitLab contribution calendar"""

    def __init__(self, repository: ContributionRepository):
        self._repository = repository

    async def execute(self) -> GitLabContributionsDTO:
        """
        Execute the use case with configured GitLab username.

        Returns:
            GitLabContributionsDTO with 365 days of contribution data

        Raises:
            ValueError: If username is invalid
            Exception: If GitLab API call fails
        """
        username_vo = Username(settings.GITLAB_USERNAME)
