<header class="main-header">
    <button class="button--reset button--menu<?= ($getPage->template === 'home' ? ' button--home' : ''); ?>"
            id="openMenu">
    ...
    <section class="project__container" id="projects">
        <? foreach (Functions::getProjects(false) as $project) { ?>
            <article class="project__item" data-slug="/project/<?= $project->slug; ?>"
                     data-name="<?= $project->name; ?>"
                     data-image="<?= Functions::getImage($project->slug, "featured", ".png"); ?>">
                <a href="/project/<?= $project->slug; ?>" class="project__link">
                    <figure class="figure--reset">
                        <img src="<?= Functions::getImage($project->slug, "featured", ".png"); ?>"
                             class="project__image" alt="Project 1">
